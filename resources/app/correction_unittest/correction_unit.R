    ...
    sidebarLayout(
      div(
        ...
        sidebarPanel(
          tabPanel(
            "Initial input",
            uiOutput(("correctButton")), #With correction or skip correction
            uiOutput(('modelSelectedOutput')), #Raw model or the best model
            uiOutput(("sourceAdjustFor")), #Adjust for a selected element
            uiOutput(("sourceRemove")), #Remove from adjustment
            
            #add p value of the equation
            uiOutput(("sourceEq")),
            uiOutput(("sourceShapiro")), #Shapiro test p value of residuals
            
            
            uiOutput(("sourceCorrection")), #R2 value
            uiOutput(("standardDiv")), #VIF
            
            
            uiOutput(("pvalueDW")),
            uiOutput(("performDW")), #Durbin watson test #need p value adjustment
            
            uiOutput(("pvalueBP")),
            uiOutput(("performBP")), #Breush pagan test #need p value adjustment
            
            
            uiOutput(("CooksD")), #Cooks distance
            uiOutput(("sourceApplyCorrelation")), #Button
            br(),
            br()
          )
        )
      ...
      mainPanel(
        ...
        fluidPage(
          tabsetPanel(
            ...
            tabPanel(
              "All Options",
              use_busy_spinner(spin = "pixel", color = "#1230bf", margi = c(10, 10), spin_id = NULL, height = "250px",
                               width = "250px", position = "bottom-right"),
              
              box(
                title = "Available option", status = "success", height =
                  "auto", solidHeader = T, width = "12",
                column(
                  width = 12,
                  shinycssloaders::withSpinner(DTOutput(("resOutput"))), style = "height:'500px'; overflow-y: scroll;overflow-x: scroll;"
                )
              )
            ),
            tabPanel (
            ...
            tabPanel (
              ...
              br (),
              br (),
              downloadButton (('downloadSourceCorrected'), 'Download')
            ),
            tabPanel (
              'Plot traformed & untraformed results',
              div(style="display: inline-block;vertical-align:top; width: 150px;",uiOutput (('selectTarget2'))),
              div(style="display: inline-block;vertical-align:top; width: 150px;",uiOutput (('selectPredictor'))),
              div(style="display: inline-block;vertical-align:top; width: 150px;",uiOutput (('selectRespoe'))),
              box (
              ...
              box (
                title = "Plot of traformed & untraformed sources for selected Target", status = "primary",
                height = 870L, width = "12", solidHeader = T,
                column(
                  width = 12,
                  plotlyOutput(('traformedPlot'))
                )
              )
            )
          )
...
server <- function(input, output, session){
  ...
  sourceData <- reactive ({
    read.csv ("allen_source.csv")
  })
  
  targetData <- reactive ({
    read.csv ("allen_target.csv")
  })
  
  output$sourceData <- renderDT({
    datatable(sourceData(),options = list(pageLength = 100))
  })
  
  output$targetData <- renderDT ({
    datatable(targetData(),options = list(pageLength = 100))
  })
  ...
  selectedOutTable <- eventReactive (input$sourceApplyCorrelation,{
    req (selectedOut ())
    print ("starting selectedCorrectionTabs")
    dat <- selectedCorrectionTabs (selectedOut(), myTarget(), input$modelSelectedInput)
    print ("passed selectedCorrectionTabs")
    dat 
  })
  ...
  correctedListData <- eventReactive ( input$sourceApplyCorrelation, {
    ...
    print ("preparing for getListCorrected Data")
    req (selectedOut())
    ...
    if (any(is.na(zeroTracers))){zeroTracers <- NA}
    if (any(is.na(mixed))){mixed <- NA}
    if (any(is.na(negatives))){negatives <- NA}
