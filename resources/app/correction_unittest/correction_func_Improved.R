...
selectedCorrectionTabs <- function (correctionList, target, corFor){
  ...
  for (i in seq(1, length(correctionList))) {
    ...
    if (corFor == 'rawM'){
      ...
      correctionList[[i]] <- data.frame(datas[which(datas$rank == 1), ])
    }
...
getListCorrectedData <- function (correctionList, target, x, corFor, pickModel, neg_glob, zeroTracers, mixed, negatives){
  ...
  print ("finished first half get list Corrected data")
  ...
  print ("finished second half get list Corrected data")
  ##FIX it
  # 
  # mixed <- names (x)[mixed]
  ...
  print ("finished conversion getlist Corrected data")
  correctedDataFrames
}
